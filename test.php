...
// retrieves candidate id to be updated from candidate_details.php
$upd_id = $_GET['upd'];

// sending candidate id to be updated to candidate_update_2.php
session_start();
$_SESSION['upd_id']=$upd_id;

// Fetching existing candidate information
$sql = "SELECT * FROM candidate_information WHERE candidate_id='$upd_id'";
$res= $conn->query($sql);
...
$workexperience = $updData['experience_in_years'];
...
<head>
    ...
    <script>
      ...
      function checkForm()
      {
        var fname = document.forms["candidate_upd"]["fullname"].value;
        var age = document.forms["candidate_upd"]["age"].value;
        var email = document.forms["candidate_upd"]["email"].value;
        var phnum = document.forms["candidate_upd"]["phnumber"].value;
...
<h2>Update your details here:</h2><br>
    ...
    <form name = "candidate_upd" onsubmit = "return checkForm()" href="candidate_update_2.php?upd=<?php echo $upd_id; ?>" action="candidate_update_2.php" method="POST">
        ...
        <label for="phnum" class="form-label">Phone number: </label>
          <input type = "tel" class="form-control" id="phnum" name="phnumber" value = "<?php echo $phnumber; ?>" required>
        </div>
        <br>
        ...
        <label for="workexp" class="form-label">Total work experience (in years): </label>
          <input type = "text" class="form-control" id="workexp" name="workexperience" value = "<?php echo $workexperience; ?>" required>
        </div>
        <br>
